...
conn = pyodbc.connect('Driver={SQL Server};'
                      'Server=10.170.139.191\SQLEXPRESS;'
                      'Database=helpdesk;'
                      'Trusted_Connection=Yes;')

data = pd.read_sql_query('SELECT * from incident', conn)

df1 = data[['assigned_to', 'category', 'calendar_stc', 'subcategory',
            'contact_type', 'sys_created_on', 'caller_id', 'state', 'priority', ]]
...
while x < ser.size:
    ...
    if(name == df1.loc[x][0]):
        count += 1
        ticket = incident(x, df1.loc[x][0], df1.loc[x][6], df1.loc[x][5], df1.loc[x][4],
                          df1.loc[x][7], df1.loc[x][8], df1.loc[x][3], df1.loc[x][1], df1.loc[x][2])
        ...
        employeeList.append(employee(name, x, count, accessCount, helpCount, failCount, accessTime, helpTime, failTime, sub, contactType,
                            helpEmail, helpPhone, helpOther, accessEmail, accessPhone, accessOther, failEmail, failPhone, failOther, createdOn, tickets))
        name = df1.loc[x][0]
        tickets = []
        count = 0
        time = 0
...
employeeList.append(employee(name, x, count, accessCount, helpCount, failCount, accessTime, helpTime, failTime, sub, contactType,
                    helpEmail, helpPhone, helpOther, acceessEmail, accessPhone, accessOther, failEmail, failPhone, failOther, createdOn, tickets))
